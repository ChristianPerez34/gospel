...
- **Default Variant**: The absence of a Model Variant for a provider/model selection. It runs the parent model without variant-specific request parameters and appears before named variants wherever variants are offered.
- **Model Selection**: A user-facing picker row representing either a parent model's Default Variant or one of its named Model Variants. Model Selection rows are flat even when multiple rows share the same provider/model slug.
- **Session Model Selection**: The provider, model, and optional Model Variant ID stored on a Session for future turns. A Session stores the variant ID, not raw provider parameters; if the ID no longer resolves, the backend falls back to the Default Variant with a non-blocking warning.
...
- **ReviewEngine**: The backend orchestration module that executes single-focus and multi-focus code reviews. It sits at a clean seam accepting a `ReviewRequest` and orchestrating single-focus and multi-focus code review pipelines behind a single interface (`execute` and `execute_multi`).
...
- **Turn Persistence Adapter**: A backend adapter sitting at the persistence seam of Session Turn execution. It encapsulates SQLite storage operations for Display Transcript updates, Model History preservation, Draft Session activation, and failure snapshotting behind a testable storage interface.
