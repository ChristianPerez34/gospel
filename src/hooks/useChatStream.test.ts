...
interface CorpusAutoBuildCompletePayload {
  success: boolean;
  symbol_count: number;
}
...
type EventPayloads = {
  "llm-token": string;
  "llm-done": string | LlmDonePayload;
  "llm-error": { code: string; message: string };
  "llm-tool-call": ToolCallPayload;
  "llm-tool-result": ToolResultPayload;
  "llm-reasoning": ReasoningPayload;
  "corpus-auto-build-complete": CorpusAutoBuildCompletePayload;
  "llm-model-variant-warning": ModelVariantWarningPayload;
  "approval-requested": ApprovalRequest;
  "approval-resolved": ApprovalResolution;
};

type EventName = keyof EventPayloads;
type ListenerCallback<T> = (event: { payload: T }) => void;
type CapturedListeners = {
  [K in EventName]?: Array<ListenerCallback<EventPayloads[K]>>;
};
...
function triggerEvent<K extends EventName>(eventName: K, payload: EventPayloads[K]) {
...
describe("useChatStream", () => {
  beforeEach(() => {
    ...
    vi.mocked(listen).mockImplementation(async (eventName, callback) => {
      ...
      let listeners = capturedListeners[name] as
        | Array<ListenerCallback<EventPayloads[typeof name]>>
        | undefined;
      if (!listeners) {
        listeners = [];
        capturedListeners[name] = listeners;
      }
      listeners.push(callback as ListenerCallback<EventPayloads[typeof name]>);
      const unlisten = () => {
        unlistenCalls += 1;
      };
  ...
  describe("token and done listeners", () => {
    ...
    it("llm-done finalizes the turn, strips reasoning blocks, calls onMessages", async () => {
      ...
      await act(async () => {
        triggerEvent("llm-done", "final-answer");
      });
  ...
  describe("tool and approval lifecycle", () => {
    ...
    it("approval-requested adds a pending card; approval-resolved transitions it", async () => {
    ...
    it("resolveApproval invokes the resolver on every call; the event transitions the card once", async () => {
      const onResolveApproval = vi.fn().mockResolvedValue(undefined);
      const { result } = renderChatStream({ onResolveApproval });
      await act(async () => {});
